...
void VppEntry::read(std::vector<std::uint8_t> data, QTextEdit *log)
{
        ...
        memcpy(&header, data.data(), sizeof(VppHeader));
        ...
        std::vector<VppDirectoryEntry> dirEntries(header.fileCount);
        memcpy(
                dirEntries.data(), &data[CHUNK_SIZE],
                header.fileCount * sizeof(VppDirectoryEntry));
